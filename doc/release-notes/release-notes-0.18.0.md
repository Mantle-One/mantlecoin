Mantle Core version 0.18.0 is now available from:

  <https://mantlecore.org/bin/mantle-core-0.18.0/>
...
`/Applications/Mantle-Qt` (on Mac) or `mantled`/`mantle-qt` (on
Linux).
...
recommended to use Mantle Core on unsupported systems.

Mantle Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Mantle Core does not yet change appearance when
...
`contrib/init/mantled.service`, it has been changed to use
`/var/lib/mantled` as the data directory instead of
`~mantle/.mantle`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/mantled` will
exist has enough space (check using `df -h /var/lib/mantled`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/mantle/mantle/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://mantlecore.org/en/contact/) the Mantle Core
...
The systemd init file (`contrib/init/mantled.service`) has been changed
to use `/var/lib/mantled` as the data directory instead of
`~mantle/.mantle`. This change makes Mantle Core more consistent with
...
`datadir` option in `/etc/mantle/mantle.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/mantle/mantle.conf`.


Documentation
-------------

- A new short [document](https://github.com/mantle/mantle/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/mantle/mantle/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/mantle/mantle/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/mantle/mantle/blob/master/doc/mantle-conf.md)
  about the `mantle.conf` file describes how to use it to configure
  Mantle Core.

- A new document introduces Mantle Core's BIP174 [Partially-Signed
  Mantle Transactions
  (PSBT)](https://github.com/mantle/mantle/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/mantle/mantle/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Mantle-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Mantle Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/mantle/mantle/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/mantle/mantle/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Mantle Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Mantle Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/mantle/mantle/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/mantle/mantle/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart mantled with the `-deprecatedrpc=generate`
...
- A new `mantle-wallet` tool is now distributed alongside Mantle
...
This section describes planned changes to Mantle Core that may affect
other Mantle software and services.

- Since version 0.16.0, Mantle Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Mantle Core 0.20 (expected about
  a year after 0.18), Mantle Core will default to native segwit
  ...
  to bech32 addresses, and if the Mantle Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Mantle Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Mantle Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Mantle Core
...
- A sub-project of Mantle Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Mantle Core.  See their [project
  page](https://github.com/mantle-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Mantle Core's own implementation, although entropy
  gathered by Mantle Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Mantle Core a
...
- On macOS, Mantle Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `mantle_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libmantle_server.a` (jnewbery)
...
- #13926 [Tools] mantle-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like mantled (MarcoFalke)
...
- #14521 qt, docs: Fix `mantle-qt -version` output formatting (hebasto)
...
- #11625 Add MantleApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libmantle_server` (laanwj)
...
- #15154 Configure: mantle-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/mantle-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Mantle Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libmantle_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in mantle-util-test.py (isghe)
...
- #12775 Integration of property based testing into Mantle Core (Christewart)
...
- #12246 Bugfix: Only run mantle-tx tests when mantle-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_mantle` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_mantle` (MarcoFalke)
...
- #14186 mantle-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_mantle.conf` message on startup if conf file exists (leishman)
...
- #12255 Update mantle.service to conform to init.md (dongcarl)
...
- #14497 Add doc/mantle-conf.md (hebasto)
...
- #15354 Add missing `mantle-wallet` tool manpages (MarcoFalke)
...
- #15765 correct mantleconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support mantle folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/mantle/).
