...
Mantle Core version 0.20.0 is now available from:

  <https://mantlecore.org/bin/mantle-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Mantle Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Mantle Core version 0.18.0.
...
- Testing or debugging of implementations of the Mantle P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Mantle Core. Mantle Core logs debug messages
...
- OpenSSL is no longer used by Mantle Core.  (#17265)

- BIP70 support has been fully removed from Mantle Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/mantle/mantle/issues/16599),
  [PR #16702](https://github.com/mantle/mantle/pull/16702), and the
  `mantled help` for more information.  This option is experimental and
...
- All custom settings configured when Mantle Core starts are now
...
- The "Start Mantle Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://mantlecore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Mantle Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Mantle Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/mantle-core/HWI) for
  signing.  Future versions of Mantle Core should support a GUI option
...
- Mantle Core's automatically-generated source code documentation is
  now available at https://doxygen.mantlecore.org.  (#17596)
...
- The `mantle-cli` utility used with the `-getinfo` parameter now
...
- The `mantle-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=7333` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `mantlestrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_mantle-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBMANTLE_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_mantle` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBMANTLE_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for mantle-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate mantle schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest mantle.conf (sdaftuar)
...
- #18022 Fix appveyor `test_mantle` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (mantle-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone mantle-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Mantle Core files (hebasto)
...
- #17119 Fix broken mantle-cli examples (andrewtoth)
...
- #17503 Remove bitness from mantle-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.mantlecore.org (laanwj)
...
- #17648 Rename wallet-tool references to mantle-wallet (hel-o)
...
- #18341 Replace remaining literal MAC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On mantled startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from mantle-cli -getinfo (malevolent)
