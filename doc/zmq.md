...
The Mantle Core daemon can be configured to act as a trusted "border
router", implementing the mantle wire protocol and relay, making
...
The ZeroMQ feature in Mantle Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building mantled:
...
For instance:
    ...
    $ mantled -zmqpubhashtx=tcp://127.0.0.1:27332 \
               -zmqpubhashtx=tcp://192.168.1.2:27332 \
               -zmqpubrawtx=ipc:///tmp/mantled.tx.raw \
...
These options can also be provided in mantle.conf.
...
From the perspective of mantled, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into mantled directly. Furthermore, no information is
...
using. Mantled appends an up-counting sequence number to each
