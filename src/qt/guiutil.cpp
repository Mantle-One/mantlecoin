...
#include <qt/mantleaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Mantle address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MantleAddressEntryValidator(parent));
    widget->setCheckValidator(new MantleAddressCheckValidator(parent));
}

bool parseMantleURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no mantle: URI
    if(!uri.isValid() || uri.scheme() != QString("mantle"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MantleUnits::parse(MantleUnits::MAC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMantleURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMantleURI(uriInstance, out);
}

QString formatMantleURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("mantle:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MantleUnits::format(MantleUnits::MAC, info.amount, false, MantleUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMantleConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", MANTLE_CONF_FILENAME));
    ...
    /* Open mantle.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mantle.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mantle (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Mantle (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Mantle*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "mantle.desktop";
    return GetAutostartDir() / strprintf("mantle-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a mantle.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Mantle\n";
        else
            optionFile << strprintf("Name=Mantle (%s)\n", chain);
