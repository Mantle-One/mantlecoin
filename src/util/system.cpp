...
const char * const MANTLE_CONF_FILENAME = "mantle.conf";
const char * const MANTLE_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/mantle/mantle/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //mantle-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", MANTLE_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Mantle
    // macOS: ~/Library/Application Support/Mantle
    // Unix-like: ~/.mantle
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mantle";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Mantle";
#else
    // Unix-like
    return pathRet / ".mantle";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MANTLE_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Mantle Core copyright is not removed by accident
    if (copyright_devs.find("Mantle Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Mantle Core developers";
    }
